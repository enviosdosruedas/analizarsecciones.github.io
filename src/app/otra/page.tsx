...
// Helper function to extract top-level directories under src/ based on estructura_proyecto2.json structure
const extractSrcDirectories = (structure: any): string[] => {
    // Expecting the root JSON structure as input
    if (!structure?.src || typeof structure.src !== 'object') {
        console.warn("Structure does not contain a 'src' object.");
        return [];
    }
    const srcObject = structure.src;
    const srcDirs = Object.keys(srcObject)
        .filter(key => {
            // Ensure the key is not '__files__' and the value is an object (directory)
            return key !== '__files__' && typeof srcObject[key] === 'object' && srcObject[key] !== null && !key.startsWith('.');
        })
        .map(dir => `src/${dir}`) // Prepend 'src/' to the directory name
...
// Helper function to extract all file paths from the project structure (estructura_proyecto2.json format) recursively
const extractAllFilePaths = (structure: any, currentPath: string = '', allPaths: ComboboxOption[] = []): ComboboxOption[] => {
    ...
    Object.keys(structure).forEach(key => {
        // Ensure processing own properties only
        ...
        if (key === '__files__' && Array.isArray(value)) {
            // Process files listed in the __files__ array
            value.forEach(filename => {
                if (typeof filename === 'string') {
                    const filePath = currentPath ? `${currentPath}/${filename}` : filename;
                    // Check if the file type is relevant
                    if (/\.(tsx|jsx|js|html|css|ts|json)$/.test(filename) && !filename.startsWith('.')) {
                        // Ensure no duplicate paths are added
                        if (!allPaths.some(p => p.value === filePath)) {
                            allPaths.push({ value: filePath, label: filePath });
                        }
                    }
                }
            });
        } else if (typeof value === 'object' && value !== null && key !== '__files__') {
            // It's a directory (and not __files__), recurse
            extractAllFilePaths(value, newPath, allPaths);
        }
        // Explicitly ignore non-object values that are not __files__ as they are handled by the __files__ array now.
    });

    // Sorting will be done in useEffect after the full list is built
    return allPaths;
};
...
const PromptForgePage: FC = () => {
  ...
  useEffect(() => {
    const fetchAndProcessStructure = async () => {
      ...
      try {
        ...
        const response = await fetch('/estructura_proyecto2.json');
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const structure = await response.json();

        // Use the updated extraction function for directories
        const extractedDirs = extractSrcDirectories(structure);

        if (extractedDirs.length > 0) {
          setDirectoryPaths(extractedDirs);
        } else {
          console.warn("Could not find directories under src in estructura_proyecto2.json");
          setDirectoryPaths([]);
          toast({
            title: 'Warning',
            description: 'Could not find project directories in estructura_proyecto2.json.',
            variant: 'destructive',
          });
        }

        // Use the updated extraction function for file paths
        const extractedFiles = extractAllFilePaths(structure);
        // Combine 'new' option with extracted file paths, sorted alphabetically
        setFilePaths(
        ...
        console.error("Failed to fetch or process project structure from estructura_proyecto2.json:", error);
        ...
        toast({
          title: 'Error',
          description: 'Failed to load project structure (v2). Please check console.',
          variant: 'destructive',
        });
      } finally {
  ...
  const generatePrompt = useCallback(() => {
    ...
    // NOTE: This prompt still references configuracion_proyecto.txt and estructura_proyecto.json
    // If those should also point to v2 or different files, this text needs updating too.
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 flex flex-col">
      <header className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-foreground">PromptForge (Otra)</h1>
        <p className="text-muted-foreground">
          Generate structured prompts for AI-powered web component generation (using estructura_proyecto2.json).
        </p>
      </header>
